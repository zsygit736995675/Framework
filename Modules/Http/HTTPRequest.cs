...
public class HTTPRequest
{
    ...
    public string SetPostData(string data)
    {
        if (postBuilder == null)
        {
    ...
    public void Start(bool isadd=false,string signature="",string ts="",string token="")
    {
        // ServicePointManager.ServerCertificateValidationCallback = RemoteCertificateValidationCallback;

       // UnityEngine.Debug.Log("Handle Http Request Start:"+ url+"   timeout"+timeout);
        ...
        if (isadd)
        {
            ...
            this.request.Headers.Add("ts", ts);
            this.request.Headers.Add("token" , token);
        }
    ...
    {
        UnityEngine.Debug.Log(string.Format("Handle Http Callback, code:{0}", code));
        if (callback != null)
        {
