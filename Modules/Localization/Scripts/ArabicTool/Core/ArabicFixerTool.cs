  ...
  internal class ArabicFixerTool
  {
    ...
    internal static string RemoveTashkeel(string str, out List<TashkeelLocation> tashkeelLocation)
    {
      ...
      for (int position = 0; position < charArray.Length; ++position)
      {
        ...
        {
          if (num > 0 && ArabicFixerTool.combineTashkeel && tashkeelLocation[num - 1].tashkeel == 'ّ')
          {
        ...
        {
          if (num > 0 && ArabicFixerTool.combineTashkeel)
          {
    ...
    internal static string FixLine(string str, bool rtl)
    {
      string str1 = "";
      List<TashkeelLocation> tashkeelLocation;
      string str2 = ArabicFixerTool.RemoveTashkeel(str, out tashkeelLocation);
      ...
      for (int index = 0; index < charArray.Length; ++index)
      {
        ...
        if (!ArabicFixerTool.IsIgnoredCharacter(charArray[index]))
        {
          if (ArabicFixerTool.IsMiddleLetter(charArray, index))
            letters[index] = (char) ((uint) charArray[index] + 3U);
          else if (ArabicFixerTool.IsFinishingLetter(charArray, index))
            letters[index] = (char) ((uint) charArray[index] + 1U);
          else if (ArabicFixerTool.IsLeadingLetter(charArray, index))
            letters[index] = (char) ((uint) charArray[index] + 2U);
        }
        str1 = str1 + Convert.ToString((int) charArray[index], 16) + " ";
        if (flag)
          ++index;
      ...
      if (ArabicFixerTool.showTashkeel)
        letters = ArabicFixerTool.ReturnTashkeel(letters, tashkeelLocation);
          ...
          if (charList2.Count > 0)
          {
            for (int index2 = 0; index2 < charList2.Count; ++index2)
            {
              if (rtl)
                ...
                charList1.Add(charList2[index2]);
            }
            charList2.Clear();
          }
      ...
      if (rtl)
      {
        for (int index = 0; index < charList1.Count; ++index)
          chArray[index] = charList1[index];
      }
      else
      {
        for (int index = charList1.Count - 1; index >= 0; --index)
          chArray[index] = charList1[charList1.Count - index - 1];
      }
