    ...
    internal class AssetBundleBuildTab
    {
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            m_CopyToStreaming = new ToggleData(
                ...
                "After build completes, will copy all build content to " + m_streamingPath + "/AssetBundles/" + ((BuildTarget)m_UserData.m_BuildTarget).ToString() + " for use in stand-alone player.",
                m_UserData.m_OnToggles);
        ...
        private void ExecuteBuild()
        {
            if (AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory) {
                ...
                if (m_ForceRebuild.state)
                {
                    ...
                    if (m_CopyToStreaming.state)
                        message += " and " + m_streamingPath + "/AssetBundles/" + ((BuildTarget)m_UserData.m_BuildTarget).ToString();
                    message += "?";
                    if (EditorUtility.DisplayDialog("File delete confirmation", message, "Yes", "No"))
                    {
                        try
                        {
                            ...
                            if (m_CopyToStreaming.state)
                                if (Directory.Exists(m_streamingPath + "/AssetBundles/" + ((BuildTarget)m_UserData.m_BuildTarget).ToString()))
                                    Directory.Delete(m_streamingPath + "/AssetBundles/" + ((BuildTarget)m_UserData.m_BuildTarget).ToString(), true);
                        }
            ...
            if(m_CopyToStreaming.state)
            {
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath + "/AssetBundles/" + ((BuildTarget)m_UserData.m_BuildTarget).ToString());
                AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);
            }
        }
